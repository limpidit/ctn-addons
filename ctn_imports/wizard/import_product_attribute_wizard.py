    ...
    def action_import_file(self):
        ...
        try:
            ...
            for row_index, row in enumerate(csv_reader, start=2):
                ...
                ref_interne = clean_row[1]  # Colonne B
                attr_name = clean_row[2]    # Colonne C
                val_ids_str = clean_row[3]  # Colonne D
                ...
                if not current_product:
                    _logger.warning("Ligne %s : Ligne ignorée. Aucun produit défini pour l'attribut '%s'.", row_index, attr_name)
                    continue
                ...
                if existing_line:
                    ...
                    _logger.info("Ligne %s : Valeurs ajoutées à l'attribut '%s' existant sur le produit [%s].", row_index, attr_name, current_product.default_code)
                else:
                    ...
                    _logger.info("Ligne %s : Nouvel attribut '%s' créé avec ses valeurs sur le produit [%s].", row_index, attr_name, current_product.default_code)
